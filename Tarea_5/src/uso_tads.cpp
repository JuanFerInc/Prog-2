...
cadena_t mayores(cadena_t cad, unsigned int k, unsigned int max){
	prio_t max_prioridad = max;
	cola_prioridad_t cp = crear_cp(k,max_prioridad);
	localizador_t loc = inicio_cadena(cad);
	//recorro toda la cadena y inserto la copia de todo los elementos que 0<= num <= max
	int top = max;
	info_t copia;

	while(es_localizador(loc)){ //O(n log(k))
		if((numero_info(info_cadena(loc,cad)) >= 0 ) && (numero_info(info_cadena(loc,cad)) <= top)){
			prio_t p = numero_info(info_cadena(loc,cad));
			if(!hay_prioridad(p,cp)){
				if(es_vacia_cp(cp)){
					copia = copia_info(info_cadena(loc,cad));	
					insertar_en_cp(copia,p,cp);
				}else if(es_llena_cp(cp)){
					if( prioridad_prioritario(cp) < p){
	...
	while(!es_vacia_cp(cp)){
		copia = copia_info(prioritario(cp));
		insertar_al_final(copia,res);
		eliminar_prioritario(cp);
	}
	liberar_cp(cp);
